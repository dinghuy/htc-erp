...
3.  **Metadata fetch**: Call `get_screen` for the target screen to retrieve the latest screenshot and HTML metadata when available.
4.  **Asset download**: Use an available web or shell tool to fetch the HTML code. In Codex, prefer `functions.shell_command` with `Invoke-WebRequest` when you need a local copy.
