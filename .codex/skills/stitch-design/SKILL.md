...
5.  **Asset Management** — When the workflow needs local artifacts, fetch the latest screen assets via `get_screen` and save them to `.stitch/designs`.
...
|:---|:---|:---|
| "Design a [page]..." | [text-to-design](workflows/text-to-design.md) | `generate_screen_from_text` |
| "Edit this [screen]..." | [edit-design](workflows/edit-design.md) | `edit_screens` |
| "Create/Update .stitch/DESIGN.md" | [generate-design-md](workflows/generate-design-md.md) | `list_screens` + `get_screen` + `Write` |
...
- **Design System**: Check for `.stitch/DESIGN.md`. If it exists, incorporate its tokens (colors, typography). If it does not exist, continue with a lightweight design-system section in the prompt and optionally suggest the `generate-design-md` workflow.
...
After any Stitch generation or edit tool call, always surface the `output_components` text description and any follow-up suggestions to the user.
