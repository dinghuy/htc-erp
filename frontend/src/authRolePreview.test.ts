...
describe('authRolePreview', () => {
  it('applies admin preview as effective role while preserving base admin identity', () => {
    ...
    expect(user.isSalesProjectManager).toBe(true);
  });
  ...
  it('builds representative project workspace navigation without expanding permissions', () => {
    ...
    expect(getRolePreviewWorkspaceNavigation(['project_manager'])).toMatchObject({ route: 'Projects', navContext: { filters: { projectStage: 'delivery', openRepresentative: true, workspaceTab: 'timeline' } } });
