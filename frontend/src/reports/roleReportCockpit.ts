...
function buildProjectItems(highlights: WorkspaceHighlight[] = []): PersonaReportPanelItem[] {
  return highlights.slice(0, 4).map((project) => ({
    ...
    badges: [
      { label: `${numberValue(project.openTaskCount)} task`, tone: 'info' as Tone },
      {
      ...
      {
        label: `${numberValue(project.pendingApprovalCount)} approval`,
...
function buildApprovalItems(approvals: ApprovalItem[] = []): PersonaReportPanelItem[] {
  return approvals.slice(0, 4).map((approval) => ({
    ...
    meta: [approval.projectCode, approval.projectName, approval.department].filter(Boolean).join(' · ') || 'Hàng đợi approval',
    badges: [
      {
...
export function buildPersonaReportCockpit(
  ...
  const defaultCards = [
    { label: 'Approval chờ', value: numberValue(approvals.summary?.pendingCount), tone: 'warn' as Tone },
  ...
  const byMode: Record<RolePersonaMode, PersonaReportCockpit> = {
    sales: {
      ...
      watchDescription: 'Hàng đợi thương mại, hồ sơ thiếu và các exception đang cản tốc độ chốt deal.',
      watchItems: [...approvalItems.slice(0, 2), ...inboxItems.slice(0, 2)],
      footerNote: 'Sales xem bức tranh commercial và handoff. Các lane finance/legal vẫn là watchlist nếu chưa có capability tương ứng.',
    },
    project_manager: {
      eyebrow: 'Cockpit PM',
      title: 'Control tower từ thương mại tới delivery',
      description: 'Một cockpit xuyên suốt từ quotation, handoff đến execution để PM không phải đổi context.',
      cards: priorityCards.length ? priorityCards : defaultCards,
      ...
      watchDescription: 'Mix giữa approval và inbox exception đang đe dọa margin hoặc tiến độ.',
      watchItems: [...approvalItems.slice(0, 2), ...inboxItems.slice(0, 2)],
      footerNote: 'PM nên thấy cả tín hiệu sales lẫn execution, nhưng quyền thao tác thực vẫn phụ thuộc capability đang có.',
    },
    procurement: {
      eyebrow: 'Cockpit mua hàng',
      title: 'Áp lực nguồn cung và rủi ro giao hàng',
      description: 'Nhìn nhanh shortage, ETA trễ, PO backlog và các exception từ vendor đang lan sang delivery.',
      cards: priorityCards.length ? priorityCards : defaultCards,
      ...
      footerNote: 'Procurement cockpit tập trung vào vendor, ETA và delivery risk; không mở sâu pricing/commercial edit.',
    },
    accounting: {
      eyebrow: 'Cockpit tài chính',
      title: 'Mốc thanh toán và approval tài chính',
      description: 'Tập trung approval tài chính, công nợ rủi ro và các hồ sơ còn thiếu ảnh hưởng invoice hoặc ERP flow.',
      cards: priorityCards.length ? priorityCards : defaultCards,
      ...
      watchItems: [...approvalItems.slice(0, 2), ...inboxItems.slice(0, 2)],
      footerNote: 'Finance cockpit là lớp review và follow-up. Quyền duyệt chỉ xuất hiện đúng lane tài chính.',
    },
    legal: {
      ...
      watchDescription: 'Legal approvals và document exceptions đang cần quyết định hoặc trả comment.',
      watchItems: [...approvalItems.slice(0, 2), ...inboxItems.slice(0, 2)],
      footerNote: 'Legal cockpit thiên về review, deviation và document completeness, không cấp quyền finance/commercial edit.',
    },
    director: {
      eyebrow: 'Cockpit điều hành',
      title: 'Control tower điều hành',
      description: 'Theo dõi risk, bottleneck và approvals vượt ngưỡng ở cấp điều hành.',
      cards: priorityCards.length ? priorityCards : defaultCards,
      ...
      footerNote: 'Director cockpit ưu tiên risk và escalations, không đi sâu thao tác vận hành hằng ngày.',
    },
    admin: {
      eyebrow: 'Giám sát admin',
      title: 'Giám sát hệ thống và tín hiệu hỗ trợ',
      description: 'Lớp quan sát toàn cục cho admin để support workflow, phân quyền và vận hành hệ thống mà không biến admin thành approver business mặc định.',
      cards: priorityCards.length ? priorityCards : defaultCards,
      ...
      watchDescription: 'Watchlist approval và inbox exception để admin hỗ trợ đúng chỗ, không vượt ranh giới quyền nghiệp vụ.',
      watchItems: [...approvalItems.slice(0, 2), ...inboxItems.slice(0, 2)],
      footerNote: 'Admin chỉ quan sát và support workflow. Các lane finance, legal, executive vẫn cần role business tương ứng.',
    },
    viewer: {
      eyebrow: 'Cockpit viewer',
      title: 'Ảnh chụp nhanh dự án read-only',
      description: 'Một màn đọc nhanh để theo dõi project, exception và approvals liên quan mà không can thiệp workflow.',
      cards: priorityCards.length ? priorityCards : defaultCards,
      focusTitle: 'Dự án đang theo dõi',
      focusDescription: 'Các project đang được theo dõi trong chế độ read-only.',
      focusItems: projectItems,
      watchTitle: 'Danh sách theo dõi read-only',
      watchDescription: 'Các approval và inbox signals liên quan để nắm tình hình.',
      watchItems: [...approvalItems.slice(0, 2), ...inboxItems.slice(0, 2)],
