...
- Cập nhật tests liên quan role preview/navigation nếu có assertion text “Finance” hoặc “Pricing”, nhưng giữ nguyên `workspaceTab: 'finance'` trong nav context tests.
...
- Không đổi key nội bộ `finance`; chỉ đổi nhãn hiển thị sang `Quản lý chi phí` để giảm churn ở nav context và role preview.
