...
export function registerQuotationReadRoutes(app: ExpressApp, deps: RegisterQuotationSubrouteDeps) {
  ...
  app.get('/api/quotations', ah(async (_req: Request, res: Response) => {
    res.json(await listQuotations());
  }));

  app.get('/api/quotations/:id', ah(async (req: Request, res: Response) => {
    const quotationId = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
    const row = await getQuotationById(quotationId);
