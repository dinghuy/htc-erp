...
export type NormalizedPricingQuotationInput = Omit<Normalized<PricingQuotationInput>, 'tpcType'> & {
  tpcType: TpcType;
};

const DEFAULT_QUOTATION: NormalizedPricingQuotationInput = {
  projectCode: '',
  customerName: '',
  supplierName: '',
  salePerson: '',
...
export const DEFAULT_RENTAL_CONFIG: Normalized<PricingRentalConfigInput> = {
  investmentQty: 2,
...
export const DEFAULT_OPERATION_CONFIG: Normalized<PricingOperationConfigInput> = {
  workingDaysMonth: 30,
  dailyHours: 20,
  movesPerDay: 70,
  kmPerMove: 1,
...
export function normalizeQuotationInput(input: PricingQuotationInput): NormalizedPricingQuotationInput {
  return {
    ...DEFAULT_QUOTATION,
    projectCode: input.projectCode ?? DEFAULT_QUOTATION.projectCode,
    customerName: input.customerName ?? DEFAULT_QUOTATION.customerName,
    supplierName: input.supplierName ?? DEFAULT_QUOTATION.supplierName,
    salePerson: input.salePerson ?? DEFAULT_QUOTATION.salePerson,
    date: input.date ?? DEFAULT_QUOTATION.date,
...
export function normalizeRentalConfig(input: PricingRentalConfigInput): Normalized<PricingRentalConfigInput> {
  return {
...
export function normalizeOperationConfig(input: PricingOperationConfigInput): Normalized<PricingOperationConfigInput> {
